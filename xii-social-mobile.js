/* XII SOCIAL — Mobile Portrait Final enhancement layer
 * Mobile-only 9:16 experience loaded after the original application modules.
...
function buildStructureTree() {
  ...
  try {
    ...
    const stageWidth = Math.max(280, Math.min(480, grid.clientWidth || window.innerWidth || 360));

    const stage = document.createElement('div');
    stage.className = 'xs-tree-stage';
    stage.style.width = `${stageWidth}px`;
    stage.style.maxWidth = '100%';
    stage.style.setProperty('--xs-tree-card-width', 'auto');
    stage.style.setProperty('--xs-tree-gap', '10px');
    ...
    if (!hint?.classList.contains('xs-tree-hint')) {
      ...
      hint.textContent = 'Struktur organisasi tersusun berakar dan otomatis menyesuaikan layar HP.';
      grid.before(hint);
    }
...
function openPublicMediaManager() {
  ...
  content.innerHTML = `
    ...
    ${createMediaCard({
      ...
      description: 'Gunakan foto vertikal rasio 9:16 agar hasil paling rapi di HP. Setelah tombol Simpan ditekan, wallpaper tersimpan di Supabase dan tampil untuk seluruh pengunjung dari Home sampai Penutup.',
...
function initialize() {
  document.documentElement.classList.add('xs-mobile-only');
  document.body.classList.add('xs-mobile-only');
